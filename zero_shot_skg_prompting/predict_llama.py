...
from dataset import ParallelTSTDatasetZSSKGLlama
...
if __name__ == '__main__':
    parser = ArgumentParser('Zero-shot SKG Prompting Model Arguments')
    ...
    train_dataset = ParallelTSTDatasetZSSKGLlama(dataset_name=dataset_name,
                                                 dataset_type='train',
                                                 tokenizer_name=model_name_map[model_name],
                                                 padding_size=max_length_map_maximum[dataset_name] + 300)
    val_dataset = ParallelTSTDatasetZSSKGLlama(dataset_name=dataset_name,
                                               dataset_type='val',
                                               tokenizer_name=model_name_map[model_name],
                                               padding_size=max_length_map_maximum[dataset_name] + 300)
    test_dataset = ParallelTSTDatasetZSSKGLlama(dataset_name=dataset_name,
                                                dataset_type='test',
                                                tokenizer_name=model_name_map[model_name],
                                                padding_size=max_length_map_maximum[dataset_name] + 300)
    ...
    for dataset_type in dataset_types:
        ...
        preds_df.to_csv(
            f'../predictions/parallel/{dataset_name}/{model_name}/{model_name_map[model_name]}-0-shot_SKG_{dataset_type}.txt',
            sep='\t', index=False)
