...
class ParallelTSTDatasetFSSKGT5(Dataset):
    """
    ...
    def __init__(self, dataset_name, dataset_type, tokenizer_name, padding_size, n_few_shot,
                 use_synonyms, use_antonyms, use_hypernyms, use_hyponyms):
        """
        ...
        :param use_synonyms: whether to include synonyms as part of the prompt
        :type use_synonyms: int
        :param use_antonyms: whether to include antonyms as part of the prompt
        :type use_antonyms: int
        :param use_hypernyms: whether to include hypernyms as part of the prompt
        :type use_hypernyms: int
        :param use_hyponyms: whether to include hyponyms as part of the prompt
        :type use_hyponyms: int
        """

        super(ParallelTSTDatasetFSSKGT5, self).__init__()
    ...
    def __prepare_samples_with_prompt__(self, samples, style_from, saliency_from):
        """
        Prepare the input prompt for the specified samples.
        :param samples: input samples
        :type samples: list(str)
        :param style_from: input style
        :type style_from: str
        :param saliency_from: input saliency name
        :type saliency_from: str
        :return: input prompts
        :rtype: list(str)
        """

        input_data_df = pd.DataFrame(samples)
    ...
    def __prepare_data_parallel__(self):
        ...
        input_data_only = self.__prepare_samples_with_prompt__(self.data[style_from].values.tolist(),
                                                               style_from, saliency_from)
        ...
        for i in range(len(samples[0])):
            samples_data_input = self.__prepare_samples_with_prompt__([sample[i][0] for sample in samples],
                                                                      style_from, saliency_from)
            samples_data_output = [sample[i][1] for sample in samples]
            samples_data = [f'{s_in}\n{s_to}: {s_out}' for s_in, s_out in zip(samples_data_input, samples_data_output)]
            input_samples.append(samples_data)

        inv_input_samples = pd.DataFrame(input_samples).T.values.tolist()
        input_data_samples = ['\n'.join(i) for i in inv_input_samples]

        input_data = [f'{s}\n{d}' for d, s in zip(input_data_only, input_data_samples)]
...
class ParallelTSTDatasetFSSKGLlama(Dataset):
    """
    ...
    def __init__(self, dataset_name, dataset_type, tokenizer_name, padding_size, n_few_shot):
        ...
        super(ParallelTSTDatasetFSSKGLlama, self).__init__()
    ...
    def __prepare_samples_with_prompt__(self, samples, style_from, saliency_from, s_from, s_to):
        """
        Prepare the input prompt for the specified samples.
        :param samples: input samples
        :type samples: list(str)
    ...
    def __prepare_data_parallel__(self):
        ...
        input_data_only = self.__prepare_samples_with_prompt__(self.data[style_from].values.tolist(),
                                                               style_from, saliency_from, s_from, s_to)
        ...
        for i in range(len(samples[0])):
            samples_data_input = self.__prepare_samples_with_prompt__([sample[i][0] for sample in samples],
                                                                      style_from, saliency_from, s_from, s_to)
            samples_data_output = [sample[i][1] for sample in samples]
            samples_data = [f'{s_in}{s_out}\n' for s_in, s_out in zip(samples_data_input, samples_data_output)]
            input_samples.append(samples_data)

        inv_input_samples = pd.DataFrame(input_samples).T.values.tolist()
        input_data_samples = ['\n'.join(i) for i in inv_input_samples]

        input_data = [f'{task_defs[self.dataset_name]}\nExamples:\n\n{s}\nNow {task_clues[self.dataset_name]}\n{d}' for
                      d, s in zip(input_data_only, input_data_samples)]
...
class ParallelTSTDatasetFSSKGGPT(Dataset):
    """
    ...
    def __init__(self, dataset_name, dataset_type, tokenizer_name, padding_size, n_few_shot):
        ...
        super(ParallelTSTDatasetFSSKGGPT, self).__init__()
