...
from dataset import ParallelTSTDatasetFSSKGLlama
...
if __name__ == '__main__':
    ...
    if 'clean' in dataset_name:
        ...
        train_dataset = ParallelTSTDatasetFSSKGLlama(dataset_name=dataset_name,
                                                     dataset_type='train',
                                                     tokenizer_name=model_name_map[model_name],
                                                     padding_size=max_length_map_maximum[dataset_name] * (
                                                             1 + n_few_shot) + 300,
                                                     n_few_shot=n_few_shot)
        val_dataset = ParallelTSTDatasetFSSKGLlama(dataset_name=dataset_name,
                                                   dataset_type='val',
                                                   tokenizer_name=model_name_map[model_name],
                                                   padding_size=max_length_map_maximum[dataset_name] * (
                                                           1 + n_few_shot) + 300,
    ...
    test_dataset = ParallelTSTDatasetFSSKGLlama(dataset_name=dataset_name,
                                                dataset_type='test',
                                                tokenizer_name=model_name_map[model_name],
                                                padding_size=max_length_map_maximum[dataset_name] * (
                                                        1 + n_few_shot) + 300,
    ...
    for dataset_type in dataset_types:
        ...
        preds_df.to_csv(
            f'../predictions/parallel/{dataset_name}/{model_name}/{model_name_map[model_name]}-{n_few_shot}-shot_SKG_{dataset_type}.txt',
            sep='\t', index=False)
